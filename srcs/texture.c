...
/*   Updated: 2016/05/27 13:33:14 by kwiessle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_texture	*create_texture(t_env *env)
{
	...
	if (!(t = (t_texture *)malloc(sizeof(t_texture))))
		return (NULL);
	t->t_1 = mlx_xpm_file_to_image(env->mlx, "./textures/wall_tag.xpm", &width, &height);
	t->tab1 = (int *)mlx_get_data_addr(t->t_1, &t->bpp, &t->size_line, &t->endian);
//	t->t_2 = mlx_xpm_file_to_image(env->mlx, "./texture/", &width, &height);
//	t->tab2 = (int *)mlx_get_data_addr(t->t_2, &t->bpp, &t->size_line, &t->endian);
//	t->t_3 = mlx_xpm_file_to_image(env->mlx, "./texture/", &width, &height);
//	t->tab3 = (int *)mlx_get_data_addr(t->t_3, &t->bpp, &t->size_line, &t->endian);
//	t->t_4 = mlx_xpm_file_to_image(env->mlx, "./texture/", &width, &height);
//	t->tab4 = (int *)mlx_get_data_addr(t->t_4, &t->bpp, &t->size_line, &t->endian);
	t->t_floor = mlx_xpm_file_to_image(env->mlx, "./textures/plasma_maze.xpm", &width, &height);
	t->floor = (int *)mlx_get_data_addr(t->t_floor, &t->bpp, &t->size_line, &t->endian);
	t->t_roof = mlx_xpm_file_to_image(env->mlx, "./textures/wall_tag.xpm", &width, &height);
